...
IORef_Storage *newIORef_Storage(int capacity) {
  ...
  retVal->lock = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
...
Value *newIORef(Value *erased, Value *input_value, Value *_world) {
  ...
  // lock mutex
  pthread_mutex_lock(global_IORef_Storage->lock);
  ...
  pthread_mutex_unlock(global_IORef_Storage->lock);
...
Value *readIORef(Value *erased, Value *_index, Value *_world) {
  ...
  pthread_mutex_lock(global_IORef_Storage->lock);
  Value * elem = global_IORef_Storage->refs[index->index];
  pthread_mutex_unlock(global_IORef_Storage->lock);
  return newReference(elem);
...
Value * sysCodegen()
{
    return (Value*)makeString("RefC Backend");
}
