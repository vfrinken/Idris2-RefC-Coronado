...
{
    int textLength = strlen(old);
    char * retVal = (char*)malloc(textLength+1);
    memcpy(retVal, old, textLength);
    retVal[textLength] = '\0';
    return retVal;
}
...
char* idris2_strerror(int errnum) {
    return newString(strerror(errnum));
}
...
char* idris2_getArg(int n) {
    return newString(_argv[n]);
}

char* idris2_getEnvPair(int i) {
    return newString(*(environ + i));
}
