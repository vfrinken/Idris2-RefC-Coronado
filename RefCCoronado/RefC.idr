...
data ExportFunctions : Type where

record ExportFunctionDef where
    constructor MkExportFunctionDef
    external : String
    internal : String
    nrArgs   : Nat
...
extractValue _ CFInteger        varName = "((Value_Int64*)" ++ varName ++ ")->i64"
...
extractValue _ CFForeignObj     varName = "(Value*)" ++ varName
...
packCFType CFInteger       varName = "makeInt64(" ++ varName ++ ")"
...
packCFType CFString        varName = "useString(" ++ varName ++ ")"
...
packCFType CFForeignObj    varName = varName
...
-- Do an O(n) lookup if this function is one to be exported. Probably slow but the list of exported functions is
-- probably not very long anyway
addExportFunction : {auto c   : Ref Ctxt Defs}
                 -> {auto ef  : Ref ExportFunctions (List ExportFunctionDef)}
                 -> Name
                 -> Nat
                 -> (exportedFcts : List (Name, String))
                 -> Core ()
addExportFunction _ _ [] = pure ()
addExportFunction currentName nrArgs ((internalName, externalName) :: xs) = do
    True <- pure (currentName == internalName)
        | False => addExportFunction currentName nrArgs xs
    let fn = (cName !(getFullName currentName))
    update ExportFunctions $ \otherDefs => MkExportFunctionDef externalName fn nrArgs :: otherDefs
    pure ()
...
createCFunctions : {auto c   : Ref Ctxt Defs}
                ...
                -> {default [] exportedFcts : List (Name, String)}
                -> Name
                -> ANFDef
                -> Core ()
createCFunctions n (MkAFun args anf) = do
    ...
    addExportFunction n (length args) {exportedFcts}
    newTemporaryVariableLevel
...
getListUpTo: Nat -> List Nat
getListUpTo t = take t [ 0 .. ]

emitExportFunction : {auto a : Ref ArgCounter Nat}
                  -> {auto il : Ref IndentLevel Nat}
                  -> {auto f : Ref OutfileText Output}
                  -> {auto t : Ref TemporaryVariableTracker (List (List String))}
                  -> ExportFunctionDef
                  -> Core ()
emitExportFunction (MkExportFunctionDef externalName internalName nrArgs) = do
    -- let oneFewerArgs = case nrArgs of
    --                     Z     => Z
    --                     (S n) => n
    let varlist = map (\i => "Value * var_" ++ show i) (getListUpTo nrArgs)
...
"""
    ...
    decreaseIndentation
    emit EmptyFC """
    Value *(*fPtr_\{internalName})(Value_Arglist*) = \{internalName}_arglist ;
    Value *closure_\{internalName} = (Value*)makeClosureFromArglist(fPtr_\{internalName}, \{arglist});
    ...
    // otherwise, assume the function maps to IO (...)

    Value_Arglist *io_closure_arglist  = newArglist(0,1);
    io_closure_arglist->args[0]        = newReference(fct_eval);
    Value *(*fPtr_io)(Value_Arglist*)  = PrimIO_unsafePerformIO_arglist;
    Value *exprVal                     = (Value*)makeClosureFromArglist(fPtr_io, io_closure_arglist);

    removeReference(fct_eval);
    Value * retVal = trampoline(exprVal);
    return retVal;
}
...
footer : {auto a : Ref ArgCounter Nat}
      -> {auto il : Ref IndentLevel Nat}
      -> {auto f : Ref OutfileText Output}
      -> {auto h : Ref HeaderFiles (SortedSet String)}
      -> {auto ef  : Ref ExportFunctions (List ExportFunctionDef)}
...
footer False = do
    emit EmptyFC """
    // create a list of functions as library use
    """
    exportedFct <- get ExportFunctions
    traverse_ emitExportFunction exportedFct
    pure ()
...
createHeaderCode : (List ExportFunctionDef)
                -> (List String)
                -> Core (List String)
createHeaderCode [] lines = pure lines
createHeaderCode ((MkExportFunctionDef externalName _ nrArgs) :: xs) lines = do
    let varlist = map (\i => "Value * var_" ++ show i) (getListUpTo nrArgs)
    remainingLines <- createHeaderCode xs lines
    pure $ "Value * \{externalName}(\{showSep ", " varlist});" :: "" :: remainingLines


standardIOFunctions : String
standardIOFunctions =
"""
/*  Below are the functions suggested to be used for interacting with the library

// usage of RefIO required to initialize (and later cleanup)
// the global RefIO storage

void init_Idris2VM();
void cleanup_idris2VM();

// all values must be passed as Value* pointer.
...
Value_Integer *makeNat(uint64_t i);
...
// given a Value* pointer, we can extract the value
...
enum ValueType {Bits8, Bits16, Bits32, Bits64, Int8, Int16, Int32, Int64,
                Integer, Double, Char, String, Closure, Pointer, IdrisInternal};
...
generateCSourceFile : {auto c : Ref Ctxt Defs}
                   -> {default [] additionalFFILangs : List String}
                   -> {default True emitMainFunction : Bool}
                   -> {default "" libraryHeaderFile : String}
                   -> (exportedFcts:List (Name, String))
                   -> (anfs:List (Name, ANFDef))
                   -> (outn : String)
                   -> Core ()
generateCSourceFile {emitMainFunction} {libraryHeaderFile} exportedFcts anfs outn =
  do _ <- newRef ArgCounter 0
     _ <- newRef FunctionDefinitions []
     _ <- newRef TemporaryVariableTracker []
     _ <- newRef ExportFunctions []
     _ <- newRef OutfileText DList.Nil
     _ <- newRef HeaderFiles empty
     _ <- newRef IndentLevel 0
     traverse_ (uncurry $ createCFunctions {additionalFFILangs} {exportedFcts}) anfs
     header -- added after the definition traversal in order to add all encountered function defintions
     footer emitMainFunction
     ...
     False <- pure emitMainFunction
        | True => pure ()
     -- in case of compilation to library, write the header definions next
     exportFcts <- get ExportFunctions
     headerCodeLines <- createHeaderCode exportFcts []
     let headerCode = fastConcat (map (++ "\n") ("#include <runtime.h>" :: "" :: headerCodeLines))
     coreLift_ $ writeFile libraryHeaderFile (headerCode ++ standardIOFunctions)
     log "compiler.refc" 10 $ "Generated H file " ++ libraryHeaderFile
...
--      coreLift_ $ putStrLn $ "out c file is " ++ outn
...
--      compileStep2 {targetType=Exec} outobj outexec
--
...
compileExprToLib :
   ...
   -> (outfile : String)
   -> Core (Maybe String)
compileExprToLib ANF targetType c _ outputDir tm outfile =
  do let outn = outputDir </> outfile ++ ".c"
     let outobj = outputDir </> outfile ++ ".o"
     let outlib = if targetType == Shared then outputDir </> "lib" ++ outfile ++ "." ++ getLibSuffix
                                       else outputDir </> "lib" ++ outfile ++ "." ++ "a"
     let outHeader = outputDir </> outfile ++ ".h"
     coreLift_ $ putStrLn os

     coreLift_ $ mkdirAll outputDir

     cdata <- getCompileDataWith (Just "RefC") False ANF tm
     --cdata <- getCompileData False ANF tm
     let exported = exported cdata
     let anfs = anf cdata
     let emitMainFunction = if targetType == Exec then True else False
     generateCSourceFile {emitMainFunction=emitMainFunction} {libraryHeaderFile=outHeader} exported anfs outn
     Just _ <- compileCObjectFile {targetType=targetType} outn outobj
       | Nothing => pure Nothing

     -- I don't know how to probe whether we're on Linux or Mac, etc,
     -- so we have multiple compilation steps, one for each system
     compileStep2 {targetType=targetType} outobj outlib
where
    getLibSuffix : String
    getLibSuffix = case os of
            "darwin" => "dylib"
            "macos" => "dylib"
            "windows" => "dll"
            "Cygwin" => "dll"
...
compileExprToLib _ _ _ _ _ _ _ = pure Nothing
